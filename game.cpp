...
void ingame1() {
    ...
    for (int i = 0; i < pipes.size(); i++) {
        pipes[i].x -= TOC_DO_DICH_CHUYEN_CUA_ONG1;

        if (pipes[i].x + CHIEU_RONG_ONG < 0) {
            pipes[i].x = SCREEN_WIDTH;
            pipes[i].height = rand() % (SCREEN_HEIGHT - KHOANG_CACH_GIUA_HAI_ONG1 - 100) + 50;
            pipes[i].pass = false;
        }
        ...
        SDL_Rect Ong_duoi = { pipes[i].x, pipes[i].height + KHOANG_CACH_GIUA_HAI_ONG1, CHIEU_RONG_ONG, SCREEN_HEIGHT - pipes[i].height - KHOANG_CACH_GIUA_HAI_ONG };
...
void ingame2() {
    ...
    for (int i = 0; i < bombs.size(); i++) {
        bombs[i].x -= TOC_DO_DICH_CHUYEN_CUA_ONG;
    ...
    if (checkhealth!=0) {
        for (int i = 0; i < healths.size(); i++) {
            healths[i].x -= TOC_DO_DICH_CHUYEN_CUA_ONG;
...
void ingame3() {
    ...
    for (int i = 0; i < pipes.size(); i++) {
        pipes[i].x -= TOC_DO_DICH_CHUYEN_CUA_ONG3;
